...
before :all do
   #Starwar.delete_all
    Question.delete_all
    @question = Question.create({:id => 1, :what_am_i => 'Question', :answer_or_question => 'Are you a person?', :node_number => 1})
    @question2 = Question.create({:id => 2,:what_am_i => 'Answer', :answer_or_question => 'Obi Wan', :node_number => 2})
    @question3 = Question.create({:id => 3,:what_am_i => 'Answer', :answer_or_question => 'X-Wing', :node_number => 3})
   end

  describe 'index' do
    it 'should select and grab first node in data base' do
      expect(Bus).to receive(:get_computer_guess_info).and_return(@question)
      get :index, {:node_number => 0}
      expect(response).to redirect_to '/compguessing?category=&node_number=1'
    end

    it 'should select the index template for rendering' do
      get :index, {:node_number => 1}
